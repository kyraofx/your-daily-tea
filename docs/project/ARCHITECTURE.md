...
The OpenAI model is configurable and defaults to the cost-sensitive `gpt-5.6-luna` with reasoning disabled during retrieval calibration. The initial discovery registry uses 36 publisher and primary-source feeds, with at least two configured sources for every section and a live health audit. Category evaluation and final review reference supplied records through temporary schema-constrained IDs; deterministic code resolves those IDs back to exact source records, so the model never controls stored URLs or attribution. Deterministic source tiers provide the source-quality score. Desk-specific feeds carry a stable publisher identity so section- and edition-level source caps prevent one publisher from dominating the briefing. Source contracts, other API plans, and licensing arrangements are TBD. The system should publish original short summaries, attribution, and links rather than reproduce articles wholesale.
