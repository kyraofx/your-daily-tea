...
The OpenAI model is configurable and defaults to the cost-sensitive `gpt-5.6-luna` with reasoning disabled during retrieval calibration. The initial discovery registry uses 36 publisher and primary-source feeds, with at least two configured sources for every section and a live health audit. Deterministic source tiers provide the source-quality score, and AI output is grounded back to supplied feed records before it can enter selection. Desk-specific feeds carry a stable publisher identity so section- and edition-level source caps prevent one publisher from dominating the briefing. Source contracts, other API plans, and licensing arrangements are TBD. The system should publish original short summaries, attribution, and links rather than reproduce articles wholesale.
