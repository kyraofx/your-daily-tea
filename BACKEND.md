...
`pnpm newsroom:generate-edition -- --date YYYY-MM-DD` runs the complete read-only calibration pipeline for all 15 sections and writes private checkpoints, candidates, a deduplication report, a review report, and a manifest under `work/edition-YYYY-MM-DD/`. Add `--resume` after a transient failure to reuse completed category checkpoints without repeating Luna calls. This command never saves or publishes; use the separate runner with `--save` only after reviewing the generated report.
