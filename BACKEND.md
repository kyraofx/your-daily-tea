...
The final whole-edition review uses the same pattern with temporary `story-###` IDs. Duplicate references are also expressed as supplied IDs and resolved back to exact source URLs only after validation. This prevents harmless model URL edits from stopping publication without introducing fuzzy URL matching or allowing a different article into the edition. The reviewer sees the complete eligible candidate pool rather than only an initial two-story reservation, so deterministic balancing can use a reviewed alternate when a story is removed or moved. Duplicate-event decisions remain binding, but deterministic placement may swap which reviewed copy is retained when doing so reduces section deficits without creating a new deficit; only one copy can remain. Category moves and broad-review low-value votes are advisory at the section floor: deterministic arbitration may retain the highest-scoring candidates already approved by the specialist category evaluator and deterministic score gate, but only until the section reaches two. Balanced selection reserves two opportunities for every section before assigning third and fourth stories, prioritizing sections with the fewest eligible alternatives so earlier sections cannot exhaust shared publisher allowances. The final publication gate then requires at least two qualifying stories in every section; it never weakens provenance, duplicate, or source-diversity standards to meet the minimum.
