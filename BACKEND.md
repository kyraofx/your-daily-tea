...
`pnpm newsroom:audit-sources` verifies that every section has at least two configured sources. Add `-- --live` to fetch and parse all feeds; the command fails if a source is unavailable or returns no parseable items. The registry contains 56 feeds and provides 2–11 sources per section, including dedicated internet-culture, social-platform, education, housing, workplace, careers, gaming, politics, health, and primary labor sources.
